...
fn subsequence_score(needle: &str, haystack: &str) -> Option<f64> {
    ...
    for value in haystack.chars() {
        if chars.get(index).is_some_and(|target| *target == value) {
            index += 1;
        } else if index > 0 {
            gaps += 1;
        }
    ...
    let mut results: Vec<_> = records
        ...
        .filter_map(|record| {
            ...
            Some(SearchResult {
                ...
                score: text_score + frequency + recency,
