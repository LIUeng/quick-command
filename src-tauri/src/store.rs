...
impl Store {
    pub fn load(path: PathBuf) -> Result<Self, String> {
        let data = if path.exists() {
            ...
            serde_json::from_slice(&bytes).map_err(|error| user_error(error, "应用数据无法解析，请重置应用数据后重试"))?
    ...
    pub fn save(&self, data: &AppData) -> Result<(), String> {
        ...
        let bytes = serde_json::to_vec_pretty(data).map_err(|error| user_error(error, "无法生成应用数据"))?;
